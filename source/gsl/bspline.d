...
version(GSL_DISABLE_DEPRECATED){
...
int gsl_bspline_eval(const(double) x, gsl_vector* B, 
                 gsl_bspline_workspace* w);

int gsl_bspline_eval_nonzero(const(double) x,
                         gsl_vector* Bk,
                         size_t* istart,
                         size_t* iend,
                         gsl_bspline_workspace* w);

int gsl_bspline_deriv_eval(const(double) x,
                       const(size_t) nderiv,
                       gsl_matrix* dB,
                       gsl_bspline_workspace* w);

int gsl_bspline_deriv_eval_nonzero(const(double) x,
                               const(size_t) nderiv,
                               gsl_matrix* dB,
                               size_t* istart,
                               size_t* iend,
